...
- [ ] (P1) WS-P2.16 — PR-binding chain: enforce unit-envelope capability vocabulary at ingress, make factory-runner POST its PR binding before submit, then add the EXECUTING→SUBMITTED binding guard. Deploy order is mandatory (runner before guard) or the factory halts. Wave 2 (WS-P2.5/P2.6) depends on this link. Plan: ~/docs/software-delivery-system/2026-07-12-wsp215-adversarial-review-and-wsp216-handoff.md — added 2026-07-12
...
- [ ] (P1) Vocabulary mismatch: intent packages declare evidence_type 'automated_test' but the verifier's DETERMINISTIC_TYPES are {test, tests, pytest, runner.verification, gate.summary, ...} — 'automated_test' is in NEITHER that set nor JUDGMENT_TYPES, so evaluate_criterion falls through to judgment_required for EVERY automated AC, no matter how good the evidence. That is the root cause of the known 'judgment_required ACs must be passed out-of-band via the verifier M2M credential' gap: it is not a UI gap, it is a vocabulary gap. Same family as the package-authority -> unit-capability projection (ADR-0001). Fix: map the intent-package evidence vocabulary to the verifier's, or add automated_test to DETERMINISTIC_TYPES — added 2026-07-12 — added 2026-07-12
## Future plans
