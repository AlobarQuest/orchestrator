...
- [ ] (P3) WS-P2.7 Inc-1 (outbound tracker projection) deferred Minors, from the final Opus whole-branch review (non-blocking; fold into WS-P2.7 Inc 2): assert `seen==[]` in `test_write_to_a_transition_path_is_forbidden`; full-`Action`-tuple asserts on the two `projection.py` skip-tests; tests for `TodoistProjector.update_item` url-fallback + non-2xx and for `cli.py`'s two env-token-guard exits; a WORKER-credential case on the `GET /api/v1/tracker-bindings` test (proves auth-only, not just SYSTEM); the create-then-record non-atomicity (a Todoist create that succeeds before the binding write fails leaves a duplicate card next pass — tracker-tidiness only, never canonical; noted in ADR-0003 cost). Closeout: `~/docs/software-delivery-system/2026-07-26-wsp27-inc1-closeout-evidence.md`; Inc-2 handoff: `~/docs/software-delivery-system/2026-07-27-wsp27-inc2-inbound-handoff-prompt.md` — added 2026-07-27
