...
- [ ] (P3) infraops-mcp-server's drift-observation CLI (`src/cli/orchestrator-cli.ts`, `AlobarQuest/infraops-mcp-server`) has no vitest coverage for its fail-open per-instance posting loop, the `--dry-run` no-post path, or its non-zero exit code — verified only by reading control flow (Task 3 review) and by the Task 6 production drills, which are not regression tests and would not catch a refactor that moved the try/catch outside the loop or reordered `makeClient()` before the dry-run check. Fail-open is a stated safety property of WS-P3.0 (the drift loop must never be hostage to the orchestrator being reachable), so an untested safety property is worth closing with real test coverage. — added 2026-07-28
## Future plans
