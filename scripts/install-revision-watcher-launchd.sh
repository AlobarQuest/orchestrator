#!/usr/bin/env bash
# Install (or reinstall) the hourly revision-watcher LaunchAgent.
#
...
#   launchctl list | grep revision-watcher
#   /bin/bash scripts/run-revision-watcher.sh --dry-run   # writes nothing; prints what it would
#
# Uninstall with:
#   launchctl bootout "gui/$(id -u)/com.devon.revision-watcher"
#   rm ~/Library/LaunchAgents/com.devon.revision-watcher.plist
set -euo pipefail

LABEL="com.devon.revision-watcher"
...
if [ ! -x "$REPO_ROOT/.venv/bin/revision-watcher" ]; then
  echo "FATAL: $REPO_ROOT/.venv/bin/revision-watcher is missing. Run: uv sync --frozen" >&2
  exit 1
fi
