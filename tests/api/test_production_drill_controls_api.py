...
def test_state_projection_is_run_scoped_and_workers_are_rejected(db_client: TestClient) -> None:
    ...
    first = db_client.post(
        ...
        headers=HUMAN,
        json={**start_body(revision_id, key="controls-first"), "lease_duration_seconds": 61},
    )
    second = db_client.post(
        "/api/v1/production-drills",
        headers=HUMAN,
        json=start_body(revision_id, key="controls-second"),
    )
...
def test_deadline_controls_are_bounded_without_mutating_global_thresholds(
    ...
    short = db_client.post(
        "/api/v1/production-drills",
        headers=HUMAN,
        json={**start_body(revision_id, key="controls-short"), "lease_duration_seconds": 0},
    )
    long = db_client.post(
        "/api/v1/production-drills",
        headers=HUMAN,
        json={
            **start_body(revision_id, key="controls-long"),
