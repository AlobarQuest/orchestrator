...
def test_verification_read_head_is_write_once(migrated_session: Session) -> None:
    ...
    armed = record_verification_read_head(
        migrated_session, actor=SYSTEM, work_unit_id=unit.id, head_sha=HEAD_A
    )
    assert armed.verification_read_head_sha == HEAD_A

    with pytest.raises(DomainError) as error:
        record_verification_read_head(
            migrated_session, actor=SYSTEM, work_unit_id=unit.id, head_sha=HEAD_B
        )
...
def test_only_the_system_actor_may_write_a_binding(migrated_session: Session) -> None:
