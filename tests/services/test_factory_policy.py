...
MALFORMED: tuple[tuple[str, str], ...] = (
    ...
    ("version is a string", VALID.replace("version = 1", 'version = "1"')),
    ("version is a boolean", VALID.replace("version = 1", "version = true")),
    ("an unknown top-level key", VALID + '\nlease = "15m"\n'),
    ("no reach table", "version = 1\n"),
...
def test_an_unknown_schema_version_fails_closed(tmp_path: Path) -> None:
    assert load_factory_policy(write(tmp_path, VALID)).version == 1  # control

    with pytest.raises(DomainError) as raised:
        load_factory_policy(write(tmp_path, VALID.replace("version = 1", "version = 2")))
...
def test_a_version_below_the_supported_one_fails_closed_too(tmp_path: Path) -> None:
    ...
    with pytest.raises(DomainError) as raised:
        load_factory_policy(write(tmp_path, VALID.replace("version = 1", "version = 0")))
