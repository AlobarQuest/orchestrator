...
vocabulary at the revision factory-runner recommends to every caller, and refuses a change
declaring a name that revision does not recognise. The reading is over the network, so it
runs as its own step rather than in the suite -- but everything it decides WITH is pure, and
pure is what gets tested here:
...
- a revision that does not recognise a declared name is named, rather than folded into a
  bare refusal.

It read TWO revisions until 2026-09-11 -- the second being the pin in this repository's own
caller workflow. ADR-0015's amendment deleted that caller, and the properties those cases
protected did not go with it: the revision reader now lives in the brief twin, and
`test_brief_consumer_compatibility.py` owns its cases.
...
def test_a_revision_that_cannot_parse_a_declared_name_is_NAMED() -> None:
    """The refusal says WHICH revision fell short, not merely that one did.

    Kept keyed by revision after 2026-09-11 left only one to read: the message a person acts
    on names the revision, and a helper shaped to carry one answer would have to be rewritten
    the next time a second becomes worth reading. Both directions are asserted, so a helper
    that reported every revision -- or none -- is caught.
    """
    served = {"repo.read", "github.pr.merge"}
    in_step = {"repo.read", "github.pr.merge"}
    behind = {"repo.read"}

    assert check.revisions_that_cannot_parse(served, {"aaa": in_step}) == {}
    assert check.revisions_that_cannot_parse(served, {"aaa": behind}) == {
        "aaa": ["github.pr.merge"]
    }
...
def test_the_comparison_names_every_capability_the_recommended_consumer_cannot_parse() -> None:
    """The guard shown firing, on the exact shape of the WS-P3.7 ordering hazard.

    This repo declares a capability; the recommended consumer does not recognise it. Shipped
