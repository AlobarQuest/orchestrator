...
class ForbiddenEndpointError(RevisionWriteError):
...
class UnusableEndpointError(RuntimeError):
    ...
    Deliberately NOT a `RevisionWriteError`: that family is what the CLI treats as costing one
    ...
    `RevisionWriteError` survives the whole suite, which is how this note came to be written rather
...
class OrchestratorClient:
    ...
    def post(self, path: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if response.status_code >= 400:
            ...
            # into one.
            raise RevisionWriteError(f"orchestrator rejected POST {path}: {response.status_code}")
        try:
            body = response.json()
            ...
            # reported as though one application had been unreadable.
            raise RevisionWriteError(
